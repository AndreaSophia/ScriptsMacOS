...
set_user_password() {
    log "Verificando/configurando contraseña para ${ALLOWED_USER}..."
    
    # Verificar si el usuario existe
    if ! /usr/bin/dscl . -read /Users/${ALLOWED_USER} >/dev/null 2>&1; then
        log "ERROR: El usuario ${ALLOWED_USER} no existe. No se puede establecer la contraseña."
        log "Sugerencia: Cree el usuario antes de ejecutar este script o mediante MDM."
        return 1
    fi

    # Decodificar contraseña
    NEW_PASS=$(echo "${ENCODED_PASS}" | /usr/bin/base64 -D)
    
    # Establecer contraseña usando dscl (requiere root)
    if /usr/bin/dscl . -passwd /Users/${ALLOWED_USER} "${NEW_PASS}" >/dev/null 2>&1; then
        log "Contraseña actualizada correctamente para ${ALLOWED_USER}."
    else
        log "ERROR: No se pudo establecer la contraseña para ${ALLOWED_USER}."
        return 1
    fi
}
...
main() {
    ...
    set_user_password || log "WARN: Continuando sin establecer contraseña."
